    ...
    for i in range(window1['prediction_window']):
        ...
        settings = {
            ...
            "log_file_name": "/suncere/pyd/PycharmProjects/output/all_city/"
                             "hourly_flaml_automl_{}_{}.log".format(city, pol['pollutant4']),
        ...
        with open(
                ...
                "hourly_Xgboost_AutoML_{}.dat".format(city, pol['pollutant4']), "wb") as file:
            pickle.dump(automl, file)

        logging.info(f"{pol['pollutant4']}({i})的最佳参数为：{automl.best_config}")   # 最好的配置
        logging.info(f"{pol['pollutant4']}({i})的最佳损失为：{automl.best_loss}")   # 最佳的损失
        logging.info(f"{pol['pollutant4']}({i})的最好结果为：{automl.best_result}")   # 训练最佳配置所花费的时间
        logging.info(f"最佳模型为：{automl.model.estimator}")
        print("{}({})最佳参数：{}".format(pol['pollutant4'], i, automl.best_config))
        print("{}({})最佳损失：{}".format(pol['pollutant4'], i, 1 - automl.best_loss))
        print("{}({})最好结果：{}".format(pol['pollutant4'], i, automl.best_result))
        print(automl.model.estimator)

        # 特征重要性
        logging.info(f"{pol['pollutant4']}的特征重要性：{automl.model.estimator.feature_importances_}")
        # plt.title("{}的特征重要性：".format(pol['pollutant4']))
        # plt.barh(x_train.columns, automl.model.estimator.feature_importances_)
        # plt.grid()
        # plt.show()
        # plt.savefig("_{}的特征重要性.png".format(pol['pollutant4']))
        # plt.close("_{}的特征重要性.png".format(pol['pollutant4']))
...
def test_model(x_test, y_test, i, city):
    ...
    model = pickle.load(open("/suncere/pyd/PycharmProjects/models/{}/"
                             "hourly_Xgboost_AutoML_{}.dat".format(city, pol['pollutant4']), "rb"))
    ...
    logging.info(f"{pol['pollutant4']}({i})的MSE:{mse}")
    logging.info(f"{pol['pollutant4']}({i})的MAE:{mae}")
    logging.info(f"{pol['pollutant4']}({i})的R2:{r2}")
    logging.info(f"{pol['pollutant4']}({i})的MAR:{mar}")
    print("{}({})的R2={}".format(pol['pollutant4'], i, r2))
    print("{}({})的mse={}".format(pol['pollutant4'], i, mse))
    print("{}({})的mae={}".format(pol['pollutant4'], i, mae))
    print("{}({})的mar={}".format(pol['pollutant4'], i, mar))
