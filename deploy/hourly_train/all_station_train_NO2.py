...
for city in station_dict:
    ...
    for number, station in zip(station_num, station_dict[city]):
        ...
        try:
            ...
            data = add_lastday_data(data, pol['pollutant2'])
    ...
    filename = "Flaml_" + pol['pollutant2'] + "_" + windows_str + "_xgboost_" + time_str + "hourly"
    logging.basicConfig(
        filename="/suncere/pyd/PycharmProjects/output/all_city_train/{}/".format(city) + filename + ".log",
        filemode="w", level=logging.INFO,
        format="%(asctime)s - %(levelname)s - %(message)s")

    logging.info(f"training_model:AutoML_{pol['pollutant2']}")
    ...
    # 删除相关性相关特征
    single_city_data = single_city_data.drop(labels=["isobaric_850_v", "isobaric_700_u",
                             "surface_SUNSD", "isobaric_500_u", "hour"], axis=1)
    ...
    # 保存特征
    logging.info(f"{pol['pollutant2']}的特征为：{single_city_data.columns[1:]}")
    logging.info("-" * 50)

    # 训练模型
    mse, mae, mar, r2 = NO2_flaml_automl.train_model(single_city_data, city)
