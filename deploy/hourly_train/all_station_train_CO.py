...
# for city in ["宁波市"]:   # 某一年没有数据
# for city in ["金华市"]:   # 没有数据
    ...
    for number, station in zip(station_num, station_dict[city]):
        ...
        try:
            ...
            # 加入相同时间点的数据
            # O3
            # data = add_lastyear_data(data, pol['pollutant'])
            data = add_lastday_data(data, pol['pollutant3'])
    ...
    filename = "Flaml_" + pol['pollutant3'] + "_" + windows_str + "_xgboost_" + time_str + "hourly"
    logging.basicConfig(
    ...
    logging.info(f"training_model:AutoML_{pol['pollutant3']}")
    ...
    # 删除相关性相关特征
    single_city_data = single_city_data.drop(labels=["isobaric_850_v", "isobaric_700_v", "surface_sp",
                                                     "surface_prate", "isobaric_500_v"], axis=1)
    ...
    # 保存特征
    logging.info(f"{pol['pollutant3']}的特征为：{single_city_data.columns[1:]}")
    logging.info("-" * 50)

    # 训练模型
    mse, mae, mar, r2 = CO_flaml_automl.train_model(single_city_data, city)
