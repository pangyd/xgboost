...
for city in station_dict:
    ...
    filename = "Flaml_" + pol['pollutant4'] + "_" + windows_str + "_xgboost_" + time_str + "hourly"
    logging.basicConfig(
        filename="/suncere/pyd/PycharmProjects/output/all_city_train/{}/".format(city) + filename + ".log",
        filemode="w", level=logging.INFO,
        format="%(asctime)s - %(levelname)s - %(message)s")

    logging.info(f"training_model:AutoML_{pol['pollutant4']}")
    ...
    # 删除相关性相关特征
    single_city_data = single_city_data.drop(labels=["atmosphere_tcc", "isobaric_850_u",
                             "isobaric_850_v", "isobaric_700_u", "isobaric_700_v", "isobaric_500_v",
                             "isobaric_500_u", "surface_prate", "year"], axis=1)
    ...
    # 保存特征
    logging.info(f"{pol['pollutant4']}的特征为：{single_city_data.columns[1:]}")
    logging.info("-" * 50)

    # 训练模型
    mse, mae, mar, r2 = O3_flaml_automl.train_model(single_city_data, city)
