...
# d1 = pd.date_range(start="{} 23:00:00".format(pd.to_datetime(pd.datetime.now().strftime("%Y-%m-%d")) + timedelta(days=-1)),
#                    end="{} 23:00:00".format(pd.to_datetime(pd.datetime.now().strftime("%Y-%m-%d")) + timedelta(days=+9)),
#                    freq="3H")
# d1 = pd.DataFrame(d1, columns=["TimePoint"])
# d2 = pd.date_range(start="{} 23:00:00".format(pd.to_datetime(pd.datetime.now().strftime("%Y-%m-%d")) + timedelta(days=-1)),
#                    end="{} 00:00:00".format(pd.to_datetime(pd.datetime.now().strftime("%Y-%m-%d")) + timedelta(days=+10)),
#                    freq="H")
...
station = pd.read_csv('station.csv', encoding='ISO-8859-1')
area = station['Area']
...
list1 = range(len(StationCode[:110]))
list2 = range(len(StationCode[110:220]))
list3 = range(len(StationCode[220:330]))
list4 = range(len(StationCode[330:440]))
list5 = range(len(StationCode[440:550]))
list6 = range(len(StationCode[550:660]))
list7 = range(len(StationCode[660:770]))
list8 = range(len(StationCode[770:880]))
list9 = range(len(StationCode[880:990]))
list10 = range(len(StationCode[990:1100]))
list11 = range(len(StationCode[1110:1220]))
list12 = range(len(StationCode[1220:1330]))
list13 = range(len(StationCode[1330:1440]))
list14 = range(len(StationCode[1440:1550]))
list15 = range(len(StationCode[1550:1660]))
list16 = range(len(StationCode[1660:]))
l = [list1, list2, list3, list4, list5, list6, list7, list8, list9, list10, list11, list12, list13, list14, list15, list16]
...
if __name__ == "__main__":
    ...
    for code, data in all_timepoint:
        ...
        sole_station = sole_station.interpolate(method="linear", limit_direction="ffill")
        print(sole_station)
        i += 1
        if i == 1:
            break
