...
filepath = "/usr/xgboost/GFS_data/{}12/".format((pd.to_datetime(pd.datetime.now()) + timedelta(days=-1)).strftime("%Y%m%d"))
# file = "gfs.0p25.2022050100.f000.grib2"
# files = os.listdir(filepath)

files = []
for i in np.arange(0, 223, 6):
    if i == 9:
        files.append("gfs.t12z.pgrb2.1p00.f00{}".format(i))
    if 10 < i < 100:
        files.append("gfs.t12z.pgrb2.1p00.f0{}".format(i))
    if i > 100:
        files.append("gfs.t12z.pgrb2.1p00.f{}".format(i))
...
def f(files):
    ...
    for file in files:
        ...
        single_timepoint = _(range(len(StationCode[1400:1600])), atmosphere_tcc, surface_hpbl, surface_sp, surface_SUNSD,
...
if __name__ == "__main__":
    ...
    all_timepoint.to_csv("/usr/xgboost/deploy/gfs_process/gfs_extract_8.csv", index=False, sep=',',encoding="utf_8_sig")
