...
def get_data():
    ...
    # 按照时间和站点排序
    pred_data.sort_values(by=["Area", "UniqueCode", "TimePoint"], inplace=True)
    ...
    # 按照站点编号分组，分别处理每一个站点
    data = data.groupby(by=["Area", "StationCode"])
...
if __name__ == "__main__":
    ...
    city_list = ["空"]
    city_list_empty = ["空"]
    hourly_predict_pol, daily_pred = pd.DataFrame(), pd.DataFrame()
    for (code, group), (code1, group1) in zip(data, la):   # code: a tuple of (area, stationcode)
            ...
            if len(os.listdir("/usr/xgboost/models/{}".format(group["Area"][0]))) != 0:
                ...
                if city == city_list[-1]:
                    hourly_predict_pol.to_csv(
                        "/mnt/output/AQMS/station/AQMS_hourly_{}.txt".format(
                            pd.datetime.now().strftime("%Y%m%d")),
                        sep=",", mode="a", header=0, index=0)
                    daily_pred.to_csv(
                        "/mnt/output/AQMS/station/AQMS_daily_{}.txt".format(
                            pd.datetime.now().strftime("%Y%m%d")),
                        sep=",", mode="a", header=0, index=0)
                    # try:
                    #     hourly_predict_pol_city.to_csv(
                    #         "/mnt/output/AQMS/city/AQMS_hourly_{}.txt".format(
                    #             pd.datetime.now().strftime("%Y%m%d")),
                    #         sep=",", mode="a", header=0, index=0)
                    #
                    #     daily_pred_city.to_csv(
                    #         "/mnt/output/AQMS/city/AQMS_daily_{}.txt".format(
                    #             pd.datetime.now().strftime("%Y%m%d")),
                    #         sep=",", mode="a", header=0, index=0)
                    # except:
                    #     continue
                    ...
                    # GFS数据加上时间
                    try:
                        single_station_gfs = gfs_data.get_group(code[1])
                        ...
                        d = d_list[0][d_list[0]["StationCode"] == code[1]]
                        ...
                        for i, d in zip(np.arange(2, 219, 24), d_list[1:]):
                            ...
                            lastyear = list(d["O3"][d["StationCode"] == code[1]])
                            if len(lastyear) == 0:
                        ...
                        citys["bool1"] = [citys["名称"][i] in group["Area"][0] for i in range(len(citys))]
                        citys["bool2"] = [group["Area"][0] in citys["名称"][i] for i in range(len(citys))]
                        if len(citys[(citys["bool1"] == True) | (citys["bool2"] == True)]) != 1:
                            pass
                        else:
                            hourly_predict_pol_city["UniqueCode"] = list(citys["城市编码"][(citys["bool1"] == True) | (citys["bool2"] == True)]) * len(
                                hourly_predict_pol_city)
                            daily_pred_city["UniqueCode"] = list(citys["城市编码"][(citys["bool1"] == True) | (citys["bool2"] == True)]) * len(daily_pred)
        ...
        # 站点没有模型，用上个城市的结果 -- 改站点编号、城市编号
        else:
            hourly_last_station = copy.deepcopy(hourly_predict_pol)
            daily_last_station = copy.deepcopy(daily_pred)
            hourly_last_city = copy.deepcopy(hourly_predict_pol_city)
            daily_last_city = copy.deepcopy(daily_pred_city)
            hourly_last_station["UniqueCode"] = [group["UniqueCode"][0]] * 240
            daily_last_station["UniqueCode"] = [group["UniqueCode"][0]] * 10

            hourly_last_station.to_csv(
                "/mnt/output/AQMS/station/AQMS_hourly_{}.txt".format(pd.datetime.now().strftime("%Y%m%d")),
                sep=",", mode="a", header=0, index=0)
            daily_last_station.to_csv(
                "/mnt/output/AQMS/station/AQMS_daily_{}.txt".format(pd.datetime.now().strftime("%Y%m%d")),
                sep=",", mode="a", header=0, index=0)

            city_empty = code[0]  # 当前的城市
            # 第一次append,第二次开始不需要
            if city_empty != city_list_empty[-1]:
                citys["bool1"] = [citys["名称"][i] in group["Area"][0] for i in range(len(citys))]
                citys["bool2"] = [group["Area"][0] in citys["名称"][i] for i in range(len(citys))]
                if len(citys[(citys["bool1"] == True) | (citys["bool2"] == True)]) != 1:
                    pass
                else:
                    hourly_predict_pol_city["UniqueCode"] = list(
                        citys["城市编码"][(citys["bool1"] == True) | (citys["bool2"] == True)]) * len(
                        hourly_predict_pol_city)
                    daily_pred_city["UniqueCode"] = list(
                        citys["城市编码"][(citys["bool1"] == True) | (citys["bool2"] == True)]) * len(daily_pred)

                    hourly_last_city.to_csv(
                        "/mnt/output/AQMS/city/AQMS_hourly_{}.txt".format(pd.datetime.now().strftime("%Y%m%d")),
                        sep=",", mode="a", header=0, index=0)
                    daily_last_city.to_csv(
                        "/mnt/output/AQMS/city/AQMS_daily_{}.txt".format(pd.datetime.now().strftime("%Y%m%d")),
                        sep=",", mode="a", header=0, index=0)
            if city_empty in city_list_empty:
                pass
            else:
                city_list_empty.append(city_empty)
