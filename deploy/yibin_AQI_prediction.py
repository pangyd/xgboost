...
def hourly_predict_data(pred_data, hourly_data, pollutant, best_model):
    ...
    if pollutant in ["SO2", "NO2", "CO"]:
        model = pickle.load(
            open(
                "/usr/xgboost/models/sichuan_model/pred1(r2)/yibin/{}/AutoML_{}_{}(gfs_yibin).dat".format(
                    pollutant,
                    pollutant,
                    best_model),
                "rb"))
        ...
        model = pickle.load(
            open(
                "/usr/xgboost/models/sichuan_model/pred1(r2)/yibin/{}/AutoML_{}_{}(r2_gfs_yibin).dat".format(
                    pollutant,
                    pollutant,
                    best_model),
                "rb"))
    ...
    for i in range(240):
        ...
        if pollutant == "SO2":
            ...
            pred_data["{}(1)".format(pollutant)][i + 2] = y_pred[-1] + 1.15
            hourly_data["{}(1)".format(pollutant)][i + 2] = y_pred[-1] + 1.15
        if pollutant == "NO2":
            y_pred = model.predict(x)  # 单行不能预测
            pred_data["{}(1)".format(pollutant)][i + 2] = y_pred[-1] + 4.89
            hourly_data["{}(1)".format(pollutant)][i + 2] = y_pred[-1] + 4.89
        if pollutant == "CO":
            y_pred = model.predict(x)  # 单行不能预测
            pred_data["{}(1)".format(pollutant)][i + 2] = y_pred[-1] + 0.04
            hourly_data["{}(1)".format(pollutant)][i + 2] = y_pred[-1] + 0.04
        if pollutant == "O3":
            y_pred = model.predict(x)  # 单行不能预测
            pred_data["{}(1)".format(pollutant)][i + 2] = y_pred[-1] + 4.4
            hourly_data["{}(1)".format(pollutant)][i + 2] = y_pred[-1] + 4.4
        if pollutant == "PM2.5":
            y_pred = model.predict(x)  # 单行不能预测
            pred_data["{}(1)".format(pollutant)][i + 2] = y_pred[-1] + 3.69
            hourly_data["{}(1)".format(pollutant)][i + 2] = y_pred[-1] + 3.69
        if pollutant == "PM10":
            y_pred = model.predict(x)  # 单行不能预测
            pred_data["{}(1)".format(pollutant)][i + 2] = y_pred[-1] + 4.94
            hourly_data["{}(1)".format(pollutant)][i + 2] = y_pred[-1] + 4.94
...
def feature_select(pred_data, pollutant):
    ...
    if pollutant == "O3":
        data = pred_data[['surface_hpbl', 'surface_sp', 'surface_SUNSD', 'surface_t', 'surface_gust', 'highCloudLayer_hcc',
                          'lowCloudLayer_lcc', 'isobaric_850_u', 'isobaric_500_gh', 'isobaric_500_u',
                          'quarter', 'month', 'hour', 'O3_lastyear', 'O3_lastday', 'O3(1)', 'StationCode',
                          'TEM_MAX_24H', 'TEM_MIN_24H']]
    if pollutant == "PM2.5":
        data = pred_data[['atmosphere_tcc', 'surface_hpbl', 'surface_sp', 'surface_t', 'surface_gust',
                          'highCloudLayer_hcc', 'middleCloudLayer_mcc', 'isobaric_700_u', 'isobaric_700_v',
                          'isobaric_500_gh', 'isobaric_500_u', 'hour', 'PM2.5(1)', 'PM2.5_lastday',
                          'O3_lastday', 'StationCode', 'TEM_MAX_24H', 'TEM_MIN_24H']]
    if pollutant == "PM10":
        data = pred_data[['atmosphere_tcc', 'surface_hpbl', 'surface_sp', 'surface_t',
                          'surface_prate', 'surface_gust', 'highCloudLayer_hcc', 'middleCloudLayer_mcc',
                          'isobaric_850_u', 'isobaric_700_u', 'isobaric_700_v', 'isobaric_500_gh', 'hour',
                          'PM10(1)', 'PM10_lastday', 'O3_lastday', 'StationCode', 'TEM_MAX_24H', 'TEM_MIN_24H']]
...
def get_weather_data(data):
    ...
    data["press_change"] = [weather_data["PRE_24H"][weather_data["CITY"] == "宜宾市"].mean()] * len(data)
    return data
...
def get_data():
    ...
    # 只取宜宾市的数据
    pred_data = pred_data[pred_data["Area"] == "宜宾市"]
...
if __name__ == "__main__":
    ...
    for (code, group), (code1, group1) in zip(data, la):
        ...
        try:
            ...
            # hourly_data["O3_lastyear"][2:26] = d2["O3"][d2["StationCode"] == code]
            # hourly_data["O3_lastyear"][26:50] = d3["O3"][d3["StationCode"] == code]
            # hourly_data["O3_lastyear"][50:74] = d4["O3"][d4["StationCode"] == code]
            # hourly_data["O3_lastyear"][74:98] = d5["O3"][d5["StationCode"] == code]
            # hourly_data["O3_lastyear"][98:122] = d6["O3"][d6["StationCode"] == code]
            # hourly_data["O3_lastyear"][122:146] = d7["O3"][d7["StationCode"] == code]
            # hourly_data["O3_lastyear"][146:170] = d8["O3"][d8["StationCode"] == code]
            # hourly_data["O3_lastyear"][170:194] = d9["O3"][d9["StationCode"] == code]
            # hourly_data["O3_lastyear"][194:218] = d10["O3"][d10["StationCode"] == code]
            # hourly_data["O3_lastyear"][218:242] = d11["O3"][d11["StationCode"] == code]
